...
const addTRM = async (req, res, next) => {
  try {
    const { firstName, lastName, email, password, birthDate, certificate } =
      req.body;
    ...
    return res
      .status(201)
      .json({ success: true, message: "Utilisateur inscrit avec succès" });
  } catch (error) {
    console.error(error);
    ...
    if (user) {
      ...
      if (hashedPassword === user.password) {
        if (user.isBlocked()) {
          return res
            .status(401)
            .json({ success: false, error: "Votre compte est bloqué" });
        }
  ...
  try {
    ...
    if (!token) {
      return res.status(401).json({
        success: false,
        error: "Token manquant dans l'en-tête Authorization",
      });
    }
